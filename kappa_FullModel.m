function [epsilon, kappa_1_p, kappa_2_p, kappa_3_p, kappa_1_pp, kappa_2_pp, kappa_1_ppp, kappa_2_ppp] = kappa_FullModel(omega_1, omega_2, lambda, plt)
%%% -----------------------------------------------------------------
...
global c1grid c2grid c1tilde c2tilde Ngrid timeseq c1alpha c1beta c2alpha c2beta I beta
...
omega_p   = @(c) (c+0.1).^3;
omega_pp = @(c) (1/(2*pi)^0.5)*exp(-0.5*(c).^2);
omega_ppp  = @(c) c.^2;
lambda_prime = 1;
...
if plt
    ...
    ylabel('$\nabla_{\omega_1}^{L^2} \mathcal{J}_2$','interpreter','latex','FontSize',15)
    figure()
    plot(c2grid, L2gradJ2,'LineWidth',2)
    xlabel('$C_2$','interpreter','latex','FontSize',15)
    ylabel('$\nabla_{\omega_2}^{L^2} \mathcal{J}_2$','interpreter','latex','FontSize',15)
end
...
if plt
    ...
    xlabel('$C_1$','interpreter','latex','FontSize',15)
    ylabel('$\nabla_{\omega_1}^{H^1} \mathcal{J}_2$','interpreter','latex','FontSize',15)
    figure()
    plot(c2grid, H1gradJ2,'LineWidth',2)
    xlabel('$C_2$','interpreter','latex','FontSize',15)
    ylabel('$\nabla_{\omega_2}^{H^1} \mathcal{J}_2$','interpreter','latex','FontSize',15)
end
...
for j = 1:max(size(epsilon))
    ...
    [c_perturbed] = forward_FullModel(omega_1 + epsilon(j).* omega_p(c1grid), omega_2, lambda);
    num1 = (J(c_perturbed,c1tilde,c2tilde,timeseq) - J(cforward,c1tilde,c2tilde,timeseq))/epsilon(j);
    
    [c_perturbed] = forward_FullModel(omega_1, omega_2 + epsilon(j).* omega_p(c2grid), lambda);
    num2 = (J(c_perturbed,c1tilde,c2tilde,timeseq) - J(cforward,c1tilde,c2tilde,timeseq))/epsilon(j);
    
    [c_perturbed] = forward_FullModel(omega_1, omega_2, lambda + epsilon(j)*lambda_prime);
    num3 = (J(c_perturbed,c1tilde,c2tilde,timeseq) - J(cforward,c1tilde,c2tilde,timeseq))/epsilon(j);
% 
%     [c_perturbed] = forward_FullModel(omega_1, omega_2, [lambda(1), lambda(2)+ epsilon(j)*lambda_prime]);
%     num4 = (J(c_perturbed,c1tilde,c2tilde,timeseq) - J(cforward,c1tilde,c2tilde,timeseq))/epsilon(j);
%     
    kappa_1_p(j) = num1/den1;
    kappa_2_p(j) = num2/den2;
    kappa_3_p(j) = num3/den3;
    %     kappa_4_p(j) = num4/den4;
end
...
for j = 1:max(size(epsilon))
    disp(epsilon(j))
    % compute directional derivative based on finite differencing
    [c_perturbed] = forward_FullModel(omega_1 + epsilon(j).* omega_pp(c1grid), omega_2, lambda);
    num1 = (J(c_perturbed,c1tilde,c2tilde,timeseq) - J(cforward,c1tilde,c2tilde,timeseq))/epsilon(j);
    
    [c_perturbed] = forward_FullModel(omega_1, omega_2 + epsilon(j).* omega_pp(c2grid), lambda);
    num2 = (J(c_perturbed,c1tilde,c2tilde,timeseq) - J(cforward,c1tilde,c2tilde,timeseq))/epsilon(j);
...
for j = 1:max(size(epsilon))
    disp(epsilon(j))
    % compute directional derivative based on finite differencing
    [c_perturbed] = forward_FullModel(omega_1 + epsilon(j).* omega_ppp(c1grid), omega_2, lambda);
    num1 = (J(c_perturbed,c1tilde,c2tilde,timeseq) - J(cforward,c1tilde,c2tilde,timeseq))/epsilon(j);
    
    [c_perturbed] = forward_FullModel(omega_1, omega_2 + epsilon(j).* omega_ppp(c2grid), lambda);
    num2 = (J(c_perturbed,c1tilde,c2tilde,timeseq) - J(cforward,c1tilde,c2tilde,timeseq))/epsilon(j);
