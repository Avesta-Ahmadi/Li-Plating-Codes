function [omega_1, omega_2, lambda, c1forward_history, c2forward_history, L2gradJ1_history, L2gradJ2_history, H1gradJ1_history, H1gradJ2_history, omega_1_history, omega_2_history, lambda_history, Jtau_history_3] = iterative_FullModel(omega_1, omega_2, lambda, plt, foldername)
%%% -----------------------------------------------------------------
% the iterative scheme of minimization
% Input:     experimental data in time
% Output:    computed omega(c1,c2)

global c1grid c2grid Ngrid timeseq Nt Feval LineMin1 LineMin2 c1alpha c1beta c2alpha c2beta I beta
...
G2_3           = 0.0;       % initialization for (Gradient dot gradient) in Fletcher Reeves CG method for lambda_1
% G2_4           = 0.0;       % initialization for (Gradient dot gradient) in Fletcher Reeves CG method for lambda_2
...
% pjpl2_history = zeros(1,1);            % partial derivaive of J with respect to lambda
...
Jtau_history_3 = zeros(1,2);          % History of cost function values for lambda_1
% Jtau_history_4 = zeros(1,2);          % History of cost function values for lambda_2
...
while (n <= MAX_ITER) && (abs(Jtau_omega_20 - Jtau_omega_2) / abs(Jtau_omega_2) > Tol_J)
    ...
    [cforward] = forward_FullModel(omega_1, omega_2, lambda);
    ...
    [cstar] = adjoint_FullModel(omega_1, omega_2, cforward, lambda);
    ...
    for i = 1:size(timeseq,1)
        ...
        gt1(i) = ((0.006*cstar(i,1)-cstar(i,2))*lambda*I(i)*om2) / ( beta(1)+beta(2)*cforward(i,1)+0.006*beta(3)*cforward(i,2)+lambda*(1-0.006*om1*om2)*I(i) );
        gt2(i) = ((0.006*cstar(i,1)-cstar(i,2))*lambda*I(i)*om1) / ( beta(4)*cforward(i,1)-beta(3)*cforward(i,2)+lambda*om1*om2*I(i) );
    end
    for i=1:Ngrid
    ...
    % computing partial derivative of J w.r.t lambda_1
    integrand = zeros(size(timeseq));
    for i = 1:size(timeseq,1)
        om1 = interp1(c1grid, omega_1, cforward(i,1),'spline');
        om2 = interp1(c2grid, omega_2, cforward(i,2),'spline');
        integrand(i) = ((0.006*om1*om2-1)*cstar(i,1) - om1*om2*cstar(i,2) )*I(i);
    end
    pjpl = trapz(timeseq, integrand);
    pjpl_history(n,1) = pjpl;
%     % computing partial derivative of J w.r.t lambda_2
%     integrand = zeros(size(timeseq));
%     for i = 1:size(timeseq,1)
%         om1 = interp1(c1grid, omega_1, cforward(i,1),'spline');
%         om2 = interp1(c2grid, omega_2, cforward(i,2),'spline');
%         integrand(i) = lambda(1)*I(i)*cstar(i,1);
%     end
%     pjpl2 = trapz(timeseq, integrand);
%     pjpl2_history(n,1) = pjpl2;
    ...
    lambda
    Jtau1 = Jeval_FullModel(tau1, omega_1, omega_2, lambda, DDl, which);
    Jtau2 = Jeval_FullModel(tau2, omega_1, omega_2, lambda, DDl, which);
    % mnbrak for omega_1
    [tau1, Jtau1, tau2, Jtau2, tau3, Jtau3] = mnbrak_FullModel(tau1, tau2, Jtau1, Jtau2, omega_1, omega_2, lambda, DDl, which, MAXITER);
    % brent for omega_1
    Jtau_lambda0 = Jtau_lambda;
    [tau_lambda1, Jtau_lambda] = Brent_FullModel(tau1, tau2, tau3, Jtau1, Jtau2, Jtau3, Tol_Brent, MAX_ITER_BRENT, omega_1, omega_2, lambda, DDl, which);
    ...
    lambda_0(1) = lambda(1);
    lambda(1) =  lambda_0(1) + tau_lambda1 * DDl;
    fprintf(fout,'   line search for lambda performed ... \n');   
    
%     % for lambda_2
%     which = 4;  % an integer (1 or 2 or 3) indicating which function or parameter is updated, omega_1 or omega_2 or lambda
%     G2_OLD_4 = G2_4;
%     G2_4 = pjpl2*pjpl2;
%     if ( n == 1 )      
%         DDl  = -pjpl2;
...
%             case 3              % Polak-Ribiere Conjugate Gradient;
%                 G_DOT_G_OLD_4 = pjpl2*pjpl2_history(n-1,1);
%                 MOMENTUM = (G2_4 - G_DOT_G_OLD_4) / G2_OLD_4;
%         end
%         DD0 = DDl;
%         DDl  = -pjpl2 + MOMENTUM * DD0;
%     end
%     tau1 = 0.0;
%     if ( n == 1 )
%         tau2 = ALPHA0;
...
%     [tau1, Jtau1, tau2, Jtau2, tau3, Jtau3] = mnbrak_FullModel(tau1, tau2, Jtau1, Jtau2, omega_1, omega_2, lambda, DDl, which, MAXITER);
%     % brent for omega_1
%     Jtau_lambda20 = Jtau_lambda2;
%     [tau_lambda2, Jtau_lambda2] = Brent_FullModel(tau1, tau2, tau3, Jtau1, Jtau2, Jtau3, Tol_Brent, MAX_ITER_BRENT, omega_1, omega_2, lambda, DDl, which);
%     Jtau_history_4(1,n) = Jtau_lambda2;
%     % update omega_1 based on the descent direction and step length (use conjugate gradient for this update)
%     lambda_0(2) = lambda(2);
%     lambda(2) =  lambda_0(2) + tau_lambda2 * DDl;
%     fprintf(fout,'   line search for lambda performed ... \n');   
    ...
    Jtau1 = Jeval_FullModel(tau1, omega_1, omega_2, lambda, DD1, which);
    Jtau2 = Jeval_FullModel(tau2, omega_1, omega_2, lambda, DD1, which);
    % mnbrak for omega_1
    [tau1, Jtau1, tau2, Jtau2, tau3, Jtau3] = mnbrak_FullModel(tau1, tau2, Jtau1, Jtau2, omega_1, omega_2, lambda, DD1, which, MAXITER);
    % brent for omega_1
    Jtau_omega_10 = Jtau_omega_1;
    [tau_omega_1, Jtau_omega_1] = Brent_FullModel(tau1, tau2, tau3, Jtau1, Jtau2, Jtau3, Tol_Brent, MAX_ITER_BRENT, omega_1, omega_2, lambda, DD1, which);
    ...
    Jtau1 = Jeval_FullModel(tau1, omega_1, omega_2, lambda, DD2, which);
    Jtau2 = Jeval_FullModel(tau2, omega_1, omega_2, lambda, DD2, which);
    [tau1, Jtau1, tau2, Jtau2, tau3, Jtau3] = mnbrak_FullModel(tau1, tau2, Jtau1, Jtau2, omega_1, omega_2, lambda, DD2, which, MAXITER);
    Jtau_omega_20 = Jtau_omega_2;
    [tau_omega_2, Jtau_omega_2] = Brent_FullModel(tau1, tau2, tau3, Jtau1, Jtau2, Jtau3, Tol_Brent, MAX_ITER_BRENT, omega_1, omega_2, lambda, DD2, which);
...
%     fprintf(fout,'        tolerance for lambda:  %12.8f > %12.8f \n', (Jtau_lambda20 - Jtau_lambda2) / abs(Jtau_lambda2), Tol_J);
...
saveas(gcf, loc);
% figure('visible','on')
% semilogy(Jtau_history_4./Jtau_history_4(1),'Marker', 'o', 'MarkerSize',8)
% %ylim([0.01,1])
% xlabel('Iteration No.','interpreter','latex','FontSize',15)
