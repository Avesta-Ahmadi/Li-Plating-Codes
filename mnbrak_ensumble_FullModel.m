...
function [AX, FA, BX, FB, CX, FC] = mnbrak_ensumble_FullModel(AX, BX, FA, FB, f1, f2, lambda, df, which, MAXITER)
%%% -----------------------------------------------------------------
...
while (FB > FA) 
  if (ICOUNT <= 8)
    ...
    FB = Jeval_ensumble_FullModel(BX, f1, f2, lambda, df, which); % evaluate cost function
    ICOUNT = ICOUNT + 1;
  else
...
%evaluation near convergence.
  ...
  FC = Jeval_ensumble_FullModel(CX, f1, f2, lambda, df, which);
...
% jumps out an IF-block.
  for dd=1:1; if ( ((BX-U) * (U-CX)) > 0.0 )
    FU = Jeval_ensumble_FullModel(U, f1, f2, lambda, df, which);
