...
function [AX, FA, BX, FB, CX, FC] = mnbrak_ensumble_OCV(AX, BX, FA, FB, beta, df, which, MAXITER)
%%% -----------------------------------------------------------------
% INPUT:    AX, BX   -->  initial guesses for x;
%           FA, FB   -->  the corresponding function evaluations of AX and BX using FUNC
%           beta     -->  the parameters that get updated in each iteration
...
while (FB > FA) 
  if (ICOUNT <= 8)
    ...
    FB = Jeval_ensumble_OCV(BX, beta, df, which); % evaluate cost function
    ICOUNT = ICOUNT + 1;
  else
...
%evaluation near convergence.
  ...
  FC = Jeval_ensumble_OCV(CX, beta, df, which);
...
% jumps out an IF-block.
  for dd=1:1; if ( ((BX-U) * (U-CX)) > 0.0 )
    FU = Jeval_ensumble_OCV(U, beta, df, which);
